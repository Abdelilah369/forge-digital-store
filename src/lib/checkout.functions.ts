...
export const createCheckoutSession = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const targetProducts = products && products.length > 0 ? products : [];
    ...
    try {
      ...
      if (session.url) return { url: session.url, id: session.id };
    } catch {
      // Graceful instant simulated checkout in demo/preview mode
    }
